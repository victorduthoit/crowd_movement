	...
	// Methods :
		...
		public void move() {
			...
			while (indIt.hasNext()) {
				...
				Vector v = new Vector();
				Vector accPeopleRepulse = ind.peopleRepulse(people);
				Vector accWallRepulse = ind.wallRepulse(room);
				Vector vDoorAttraction = ind.doorAttraction(room);
				acc.add(accPeopleRepulse);
				acc.add(accWallRepulse);
				v.add(vDoorAttraction);
				double ax = acc.getVx();
				double ay = acc.getVy();
				double vx = v.getVx() + ax*dt;
				double vy = (int) (v.getVy() + ay*dt);
				ind.setVx(vx);
				ind.setVy(vy);
				double x = ind.getX();
				double y = ind.getY();
